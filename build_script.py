...
class GitGuardSyncBuilder:
    ...
    def print_banner(self):
        ...
        banner = """
╔══════════════════════════════════════════════════════════════════════════════╗
║                       GitGuard Sync 自動建置工具 (修復版)                    ║
║                     Git 倉庫安全同步守護者 - v3.0.1                         ║
╚══════════════════════════════════════════════════════════════════════════════╝
        """
        print(banner)
        ...
        try:
            ...
            for directory in [self.dist_dir, self.build_dir]:
                if Path(directory).exists():
                    ...
                    print(f"🗑️  已移除舊目錄: {directory}")

            # 移除 spec 檔案
            for spec_file in Path('.').glob('*.spec'):
                spec_file.unlink()
                print(f"🗑️  已移除 spec 檔案: {spec_file}")
...
VSVersionInfo(
  ffi=FixedFileInfo(
    filevers=({','.join(version_parts)},0),
    prodvers=({','.join(version_parts)},0),
    mask=0x3f,
    flags=0x0,
    OS=0x40004,
    fileType=0x1,
...
"""
    ...
    def build_executable(self):
        ...
        config = self.platform_config.get(self.platform,
                                          self.platform_config['linux'])

        # 基本參數
        cmd = [
            'pyinstaller', '--onefile', '--name', self.app_name, '--distpath',
            self.dist_dir, '--workpath', self.build_dir, '--clean',
            '--noconfirm'
        ]
        ...
        hidden_imports = [
            ...
            'tkinter.messagebox', 'tkinter.scrolledtext', 'git', 'git.repo',
            'git.remote', 'requests', 'threading', 'subprocess', 'pathlib',
            'json', 're', 'webbrowser', 'datetime', 'dataclasses', 'typing',
            'os', 'sys'
        ]
        ...
        try:
            ...
            print(f"❌ 建置失敗: {e}")
            if e.stdout:
                print(f"標準輸出: {e.stdout[-500:]}")  # 只顯示最後 500 字元
            if e.stderr:
    ...
    def verify_build(self):
        ...
        if executable_path.exists():
            ...
            try:
                if self.platform == 'windows':
                    # Windows 下不進行測試，避免 GUI 彈出
                    print("   ⚠️  Windows 平台跳過執行測試")
                else:
                    print("   🧪 測試執行檔...")
                    ...
                    if test_result.returncode == 0:
                        ...
                        print("   ⚠️  執行檔測試未通過，但檔案存在")
            except:
                print("   ⚠️  無法測試執行檔，但檔案存在")

            return True
        else:
            print(f"❌ 建置驗證失敗: 找不到執行檔 {executable_path}")
            # 列出 dist 目錄內容
            if Path(self.dist_dir).exists():
                print(f"📁 {self.dist_dir} 目錄內容:")
    ...
    def cleanup_and_finalize(self):
        ...
        try:
            ...
            if not keep_cache:
                ...
                for spec_file in Path('.').glob('*.spec'):
                    spec_file.unlink()
                    print(f"🗑️  已清理 spec 檔案: {spec_file}")
...
"""
    ...
    def run(self):
        ...
        steps = [
            self.check_environment, self.install_dependencies,
            self.clean_old_builds, self.create_version_file,
            self.build_executable, self.verify_build, self.cleanup_and_finalize
        ]
